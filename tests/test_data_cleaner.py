...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        # Assert 1: 
        missing_count = result[["name", "age"]].isna().sum().sum()
        self.assertEqual(
            missing_count,
            0,
            msg="No debería haber valores faltantes en 'name' ni 'age' tras limpiar.",
        )
        ...
        self.assertLess(
            len(result),
            len(df),
            msg="El DataFrame limpio debería tener menos filas que el original.",
        )
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.drop_invalid_rows(df, cols=["name", "does_not_exist"])
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        pdt.assert_series_equal(
            result["city"],
            df_original["city"],
            obj="Columna 'city' debería mantenerse sin cambios",
        )
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        self.assertNotIn(
            120,
            result["age"].values,
            msg="El valor outlier 120 debería haber sido eliminado.",
        )
